...
import {
  ...
  Syringe, Filter, ArrowLeft, RefreshCw,
...
const CircularProgress: React.FC<{ pct: number; size?: number }> = ({ pct, size = 96 }) => {
...
const KpiCard: React.FC<{
  ...
  <div className="bg-white rounded-2xl p-4 shadow-sm border border-slate-100 flex flex-col gap-2 min-w-0 hover:shadow-md transition-shadow">
    {loading ? (
      ...
      <>
        <div className={`w-10 h-10 rounded-full flex items-center justify-center ${iconBg} shrink-0`}>{icon}</div>
        <div>
...
export const DailyProgressReport: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (user.role === 'DISTRICT_ADMIN') {
      ...
      if (user.district_id) setFilterDistrictId(String(user.district_id));
    }
  ...
  // Auto-generate once filterStateId is set
  useEffect(() => {
    if (!isAuthChecking && adminUser && filterStateId && !hasAutoGenerated) {
      setHasAutoGenerated(true);
      generateReport(filterDate, filterLevel, filterStateId, filterDistrictId);
    }
  }, [isAuthChecking, adminUser, filterStateId]);
  ...
  const handleCSV = () => {
    ...
    const csvRows = rankedRows.map((r: any, i) => [
  ...
  return (
    <div className="h-[100dvh] w-full bg-slate-50 flex flex-col overflow-hidden font-sans">
      ...
      {/* ── Header ─────────────────────────────────────────────────────── */}
      <header className="gradient-header px-5 py-3.5 flex items-center justify-between shrink-0 shadow-xl">
        <div className="flex items-center gap-3 min-w-0">
          <button onClick={() => navigate('/admin')} className="text-purple-200 hover:text-white transition-colors p-1.5 rounded-lg hover:bg-white/10 shrink-0">
            <ArrowLeft className="w-5 h-5" />
          </button>
          <div className="min-w-0">
            <div className="flex items-center gap-2 mb-0.5">
              <span className="text-[10px] font-bold text-purple-300 uppercase tracking-widest hidden sm:block">National Health Mission – Uttarakhand</span>
              <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-emerald-500/20 text-emerald-300 border border-emerald-500/30 animate-pulse">● LIVE</span>
            </div>
            <h1 className="text-lg sm:text-xl font-extrabold text-white tracking-tight leading-tight">
              HPV Vaccination — Daily Progress Report
            </h1>
            <p className="text-[11px] text-purple-300 hidden sm:block">Tracks daily &amp; cumulative HPV vaccination progress at State, Division, District, and Block levels</p>
          </div>
        </div>
        <button onClick={handleCSV} disabled={!rows.length}
          className="flex items-center gap-2 px-4 py-2 rounded-xl font-bold text-sm bg-white/15 hover:bg-white/25 text-white border border-white/20 transition-all disabled:opacity-40 shrink-0 backdrop-blur-sm">
        ...
        <div className="max-w-[1600px] mx-auto px-4 py-4 space-y-3">
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-4">
            <div className="flex flex-wrap gap-3 items-end">
              ...
              <div className="flex flex-col gap-1">
                ...
                <div className="relative">
                  <input type="date" value={filterDate} max={today}
                    ...
                    className="pl-3 pr-9 py-2.5 border border-slate-200 rounded-xl text-sm text-slate-800 font-medium bg-slate-50 focus:outline-none focus:ring-2 focus:ring-purple-500/30 focus:border-purple-400 cursor-pointer" style={{ minWidth: 160 }} />
              ...
              {/* Level */}
              <div className="flex flex-col gap-1">
                <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Select Level</label>
                <div className="relative">
                  <select value={filterLevel}
                    onChange={e => { setFilterLevel(e.target.value as any); setFilterDistrictId('ALL'); }}
                    className="pl-3 pr-9 py-2.5 border border-slate-200 rounded-xl text-sm text-slate-800 font-medium bg-slate-50 focus:outline-none focus:ring-2 focus:ring-purple-500/30 appearance-none cursor-pointer" style={{ minWidth: 140 }}>
                ...
                <div className="flex flex-col gap-1">
                  ...
                  <div className="relative">
                    <select value={filterStateId} onChange={e => { setFilterStateId(e.target.value); setFilterDistrictId('ALL'); }}
                      className="pl-3 pr-9 py-2.5 border border-slate-200 rounded-xl text-sm text-slate-800 font-medium bg-slate-50 focus:outline-none focus:ring-2 focus:ring-purple-500/30 appearance-none cursor-pointer" style={{ minWidth: 180 }}>
              ...
              <button onClick={handleGenerate} disabled={loading}
                style={{ height: 54, borderRadius: 12, minWidth: 180 }}
                className="flex items-center justify-center gap-2 px-6 font-bold text-sm text-white bg-gradient-to-r from-[#3B1C63] to-[#522B85] hover:from-[#522B85] hover:to-[#6d3aad] rounded-xl transition-all shadow-md shadow-purple-900/20 hover:scale-[1.01] active:scale-[0.99] disabled:opacity-60 cursor-pointer">
          ...
          {/* ── Second Toolbar ─────────────────────────────────────── */}
          <div className="bg-white rounded-2xl shadow-sm border border-slate-200 px-5 py-3 flex flex-wrap items-center gap-4 justify-between">
            ...
            <div className="flex items-center gap-1 bg-slate-100 rounded-xl p-1">
              {([['best', '○ Best on Top'], ['worst', '○ Worst on Top']] as [SortDir, string][]).map(([val, label]) => (
                <button key={val} onClick={() => setSortDir(val)}
                  className={`px-4 py-1.5 rounded-lg text-sm font-semibold transition-all ${sortDir === val ? 'bg-white shadow text-purple-700' : 'text-slate-500 hover:text-slate-700'}`}>
                  {label}
                </button>
              ))}
            </div>
            <div className="flex items-center gap-2">
              <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Ranked By</span>
              <div className="relative">
                <select value={rankBy} onChange={e => setRankBy(e.target.value as RankBy)}
                  className="pl-3 pr-8 py-2 border border-slate-200 rounded-xl text-sm text-slate-800 font-semibold bg-slate-50 focus:outline-none focus:ring-2 focus:ring-purple-500/30 appearance-none cursor-pointer">
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs text-slate-400">— {filterLevel} Level</span>
              </div>
              {reportGenerated && (
                <span className={`text-xs font-bold px-3 py-1 rounded-full ${tierInfo.bg} ${tierInfo.color}`}>{tierInfo.label}</span>
              )}
            </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 xl:grid-cols-7 gap-3">
              ...
              <KpiCard loading={loading} icon={<Calendar className="w-5 h-5 text-orange-600" />} iconBg="bg-orange-50"
              ...
              {/* Circular progress ring */}
              <div className="bg-white rounded-2xl p-4 shadow-sm border border-slate-100 flex flex-col items-center justify-center gap-1 hover:shadow-md transition-shadow">
                {loading ? (
                  ...
                  <>
                    <div style={{ position: 'relative', display: 'inline-flex', alignItems: 'center', justifyContent: 'center' }}>
                      <CircularProgress pct={kpis.coveragePct} size={88} />
                      <div style={{ position: 'absolute', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                        <span className="text-base font-extrabold text-slate-900">{fmt(kpis.coveragePct, 1)}%</span>
                      </div>
                    </div>
                    <div className="text-center">
                      <div className="text-xs font-bold text-slate-500">Goal Achieved</div>
                      <div className="text-[10px] text-slate-400">{tierInfo.label}</div>
                    </div>
                  </>
                )}
              </div>
              ...
              <KpiCard loading={loading} icon={<CheckCircle2 className="w-5 h-5 text-emerald-600" />} iconBg="bg-emerald-50"
                ...
                subValue={`of ${rows.length}`} subLabel="total units" />
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
            <div className="px-5 py-3 border-b border-slate-100 flex flex-wrap items-center justify-between gap-3">
              <div className="flex items-center gap-2">
                ...
                {reportGenerated && !loading && (
                  <span className="text-xs text-slate-400">• {rows.filter(r => r.has_today_report).length} reported today</span>
                )}
              </div>
              <div className="relative">
                ...
                <input type="text" placeholder="Search by name..." value={search}
                  ...
                  className="pl-9 pr-4 py-2 border border-slate-200 rounded-xl text-sm bg-slate-50 focus:outline-none focus:ring-2 focus:ring-purple-500/30 focus:border-purple-400" style={{ width: 220 }} />
              </div>
            </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm" style={{ minWidth: 1200 }}>
                <thead>
                  <tr className="gradient-header text-white text-xs">
                    <th className="px-4 py-3 text-left font-bold uppercase tracking-wider whitespace-nowrap sticky left-0 z-10 gradient-header">Reporting Unit</th>
                    <th className="px-4 py-3 text-center font-bold uppercase tracking-wider whitespace-nowrap">Last Reported</th>
                    <th className="px-4 py-3 text-right font-bold uppercase tracking-wider whitespace-nowrap">Population<br/>(13–15 Yrs)</th>
                    <th className="px-4 py-3 text-right font-bold uppercase tracking-wider whitespace-nowrap">HPV Goal<br/>(0.8%)</th>
                    <th className="px-4 py-3 text-right font-bold uppercase tracking-wider whitespace-nowrap">Sessions Today</th>
                    <th className="px-4 py-3 text-right font-bold uppercase tracking-wider whitespace-nowrap">Vaccinations<br/>Today</th>
                    <th className="px-4 py-3 text-right font-bold uppercase tracking-wider whitespace-nowrap">Sessions<br/>Cumulative</th>
                    <th className="px-4 py-3 text-right font-bold uppercase tracking-wider whitespace-nowrap">Vaccinations<br/>Cumulative</th>
                    <th className="px-4 py-3 text-right font-bold uppercase tracking-wider whitespace-nowrap">Vacc/Session</th>
                    <th className="px-4 py-3 text-center font-bold uppercase tracking-wider whitespace-nowrap">Goal Achieved %</th>
                    <th className="px-4 py-3 text-center font-bold uppercase tracking-wider whitespace-nowrap">Rank</th>
                  </tr>
                </thead>
                <tbody>
                  ...
                  paginated.length === 0 ? (
                    ...
                    return (
                      <tr key={row.id} className={`border-b border-slate-100 hover:bg-purple-50/30 transition-colors group ${rowBg}`}>
                        <td className={`px-4 py-3 font-bold text-slate-800 sticky left-0 z-[5] border-r border-slate-100 ${rowBg} group-hover:bg-purple-50/30`}>
                          {row.name}
                        </td>
                        <td className="px-4 py-3 text-center">
                          {row.has_report
                            ? <span className="text-xs font-semibold text-slate-600 bg-slate-100 px-2 py-1 rounded-lg">{fmtDate(row.last_reporting_date)}</span>
                            : <span className="text-xs text-slate-300">—</span>}
                        </td>
                        <td className="px-4 py-3 text-right font-semibold text-slate-700">{fmt(row.population)}</td>
                        <td className="px-4 py-3 text-right font-semibold text-green-700">{fmt(row.hpv_target)}</td>
                        ...
                        <td className="px-4 py-3 text-right font-semibold text-orange-600">{row.sessions_held_cumulative !== null ? fmt(row.sessions_held_cumulative) : '—'}</td>
                        <td className="px-4 py-3 text-right font-semibold text-blue-700">{row.beneficiaries_vaccinated !== null ? fmt(row.beneficiaries_vaccinated) : '—'}</td>
                        <td className="px-4 py-3 text-right font-semibold text-teal-700">{row.vaccinations_per_session !== null ? fmt(row.vaccinations_per_session, 2) : '—'}</td>
                        <td className="px-4 py-3 text-center">
                          {coveragePct !== null ? (
                            <div className="flex flex-col items-center gap-1">
                              ...
                              <span className={`text-xs font-bold ${tier.color}`}>{fmt(coveragePct, 1)}%</span>
                            </div>
                          ) : <span className="text-slate-300">—</span>}
                        </td>
                        <td className="px-4 py-3 text-center"><span className="text-sm font-black">{rankBadge}</span></td>
                      </tr>
                    );
                  })}
                </tbody>
                {!loading && paginated.length > 0 && (
                  <tfoot>
                    <tr className="border-t-2 border-[#3B1C63]/20 font-bold text-slate-800 text-xs" style={{ background: 'rgba(59,28,99,0.04)' }}>
                      <td className="px-4 py-3 font-extrabold sticky left-0 border-r border-slate-200" style={{ background: 'rgba(59,28,99,0.04)' }}>TOTAL ({rows.length})</td>
                      <td className="px-4 py-3 text-center text-slate-400">—</td>
                      <td className="px-4 py-3 text-right">{fmt(kpis.totalPop)}</td>
                      <td className="px-4 py-3 text-right text-green-700">{fmt(kpis.totalTarget)}</td>
                      <td className="px-4 py-3 text-right text-orange-700">{fmt(kpis.totalSessionsToday)}</td>
                      <td className="px-4 py-3 text-right text-blue-700">{fmt(kpis.totalVaccToday)}</td>
                      <td className="px-4 py-3 text-right text-orange-600">{fmt(kpis.totalSessionsCumm)}</td>
                      <td className="px-4 py-3 text-right text-blue-700">{fmt(kpis.totalVaccCumm)}</td>
                      <td className="px-4 py-3 text-right text-teal-700">{fmt(kpis.vaccPerSession, 2)}</td>
                      <td className="px-4 py-3 text-center">
                  ...
                  {Array.from({ length: Math.min(totalPages, 7) }, (_, i) => {
                    ...
                    return (
                      <button key={page} onClick={() => setCurrentPage(page)}
                        className={`w-8 h-8 rounded-lg text-xs font-bold transition-colors ${currentPage === page ? 'bg-purple-700 text-white' : 'border border-slate-200 text-slate-600 hover:bg-slate-100'}`}>
                        {page}
                      </button>
                    );
                  })}
          ...
          <div className="text-center text-xs text-slate-400 py-1 font-medium">
            HPV Vaccination Monitoring Portal • National Health Mission, Uttarakhand • {new Date().getFullYear()}
          </div>
        </div>
      </div>
    </div>
