...
const CircularProgress: React.FC<{ pct: number; size?: number }> = ({ pct, size = 88 }) => {
...
export const DailyProgressReport: React.FC<DailyProgressReportProps> = ({ adminUser }) => {
  ...
  useEffect(() => {
    ...
    // Set level default based on role
  ...
  // Set default state after statesList loads
  useEffect(() => {
  ...
  // Auto-generate once state is set
  useEffect(() => {
  ...
  return (
    <div className="space-y-4 flex-1 min-h-full lg:min-h-0 flex flex-col pb-4">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-extrabold text-slate-900 tracking-tight">HPV Vaccination — Daily Progress Report</h1>
          <p className="text-xs text-slate-500 mt-1">Tracks daily &amp; cumulative HPV vaccination progress at State, Division, District, and Block levels</p>
        </div>
        <button onClick={handleCSV} disabled={!rows.length}
          className="flex items-center gap-2 px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-xs font-bold shadow-sm disabled:opacity-50 transition-colors">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden flex-1 flex flex-col">
        <div className="px-5 py-3 border-b border-slate-100 flex flex-wrap items-center justify-between gap-3 shrink-0">
          <div className="flex items-center gap-2">
          ...
          <table className="w-full text-sm" style={{ minWidth: 1200 }}>
            <thead className="sticky top-0 z-10">
              <tr className="gradient-header text-white text-xs">
                <th className="px-4 py-3 text-left font-bold uppercase tracking-wider whitespace-nowrap sticky left-0 gradient-header z-20">Reporting Unit</th>
              ...
              paginated.length === 0 ? (
                ...
                return (
                  <tr key={row.id} className={`border-b border-slate-100 hover:bg-purple-50/30 transition-colors group ${rowBg}`}>
                    <td className={`px-4 py-3 font-bold text-slate-800 sticky left-0 z-[5] border-r border-slate-100 ${rowBg} group-hover:bg-purple-50/30`}>{row.name}</td>
