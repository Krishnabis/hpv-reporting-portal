...
export const VaccineStockMonitoringReport: React.FC<{ adminUser: any }> = ({ adminUser }) => {
  ...
  const [filterFromMonth, setFilterFromMonth] = useState(currentMonthStr);
  const [filterToMonth, setFilterToMonth] = useState(currentMonthStr);
  ...
  const [reportDateToLabel, setReportDateToLabel] = useState('');
  ...
  const generateReport = async () => {
    ...
    try {
      ...
      if (filterFromMonth) q.append('fromMonth', filterFromMonth);
      if (filterToMonth) q.append('toMonth', filterToMonth);
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 px-4 py-3 shrink-0">
        <div className="flex flex-wrap gap-2.5 items-end">
          ...
          <div className="flex flex-col gap-1">
            <label className="text-[9px] font-bold text-slate-400 uppercase tracking-wider">Report Month From</label>
            <input type="month" value={filterFromMonth} max={currentMonthStr}
          ...
          <div className="flex flex-col gap-1">
            <label className="text-[9px] font-bold text-slate-400 uppercase tracking-wider">Report Month To</label>
            <input type="month" value={filterToMonth} max={currentMonthStr}
      ...
      <div ref={reportRef} className="flex-1 flex flex-col min-h-0 gap-1 pb-2 bg-slate-50 rounded-xl" style={{ backgroundColor: isSavingImg ? 'white' : undefined }}>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 px-4 py-2 flex flex-wrap items-center gap-3 justify-between shrink-0">
          <div className="flex items-center gap-1.5 text-xs font-semibold text-slate-600">
            ...
            <span className="font-extrabold text-slate-900">{reportDateLabel || currentMonthStr} to {reportDateToLabel || currentMonthStr}</span>
          </div>
