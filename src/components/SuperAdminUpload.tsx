...
export const SuperAdminUpload: React.FC = () => {
  ...
  const handleDownloadTemplate = (type: 'population' | 'livedata' | 'locations' | 'vaccine_ccp' | 'stock_receive' | 'stock_issue') => {
    ...
    if (type === 'population') {
      ...
      headers = 'statename,districtlgdcode,districtname,blocklgdcode,blockname,linelisted,vaccinated,sessions_held,Date(DD-MM-YYYY)\n';
      ...
      headers = 'Date (YYYY-MM-DD),Quantity,Batch No,Batch Expiry,Manufacturer,Source Level,Source CCL ID,Source CCL Name,Destination Level,Destination CCL ID,Destination CCL Name,Remarks\n';
